...
def calc_entropy_from_data(data, bouts, ph_idx ,params):
    """
...
def main(argv):
    ...
    parser.add_argument('-bouts','--Bouts',help="Number of bouts to sample from each condition",default=50000,type=int)
    ...
    path_to_filtered_data = '/network/lustre/iss02/home/gautam.sridhar/Markov_Fish/Datasets/Ph_Data/'
    f = h5py.File(path_to_filtered_data+args.DatasetName + '.h5','r')

    ph_idx = np.array(f['MetaData/ph_idx'], dtype=int)
    pca_fish = ma.array(f['pca_raw_fish'])[:,:,:12]
    ...
    K_range = np.arange(1,11,1) #range of delays
    ...
    calc_ent = functools.partial(calc_entropy_from_data, record_list, args.Bouts, ph_idx)
