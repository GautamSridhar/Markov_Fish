...
#     if plot:
#         c_range,rho_sets,measures,ms,idx,labels_ =  optimal_partition(phi2,inv_measure,P,return_rho=True)
...
#     '''
#     Recursive subdivision of state space: at each iteration the metastable
#     state with higher measure get subdivided along its first nontrivial
#     eigenfunction
#     '''
    
#     c_range,rho_sets,measures,idx,kmeans_labels =  optimal_partition(phi2,inv_measure,P,return_rho=True)
...
#         print(len(np.unique(kmeans_labels)))
#         if save:
#             plt.savefig('rho_{}states.pdf'.format(len(np.unique(kmeans_labels))))
#         plt.show()
