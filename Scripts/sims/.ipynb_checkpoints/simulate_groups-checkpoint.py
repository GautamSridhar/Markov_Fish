...
def main(argv):
    ...
    clus,idx = np.array(np.loadtxt('/home/a/antonio-costa/Zebrafish_ms_sims/sims/iteration_indices_groups.txt')[idx],dtype=int)
    print(clus,idx,flush=True)
    
    print("Load data",flush=True)
    tmspace_clusters = np.load('/bucket/StephensU/antonio/GautamData/spectral_split_g7.npy')
    
    path_to_filtered_data = '/bucket/StephensU/antonio/GautamData/'
    ...
    f = h5py.File('/flash/StephensU/antonio/zebrafish_ms_sims/X_pos.h5','r')
    X = ma.array(f['X'])
    X[X==0] = ma.masked
    ...
    f = h5py.File('/flash/StephensU/antonio/zebrafish_ms_sims/sims_lensim_{}_group_{}_{}.h5'.format(len_sim,clus,idx),'w')
